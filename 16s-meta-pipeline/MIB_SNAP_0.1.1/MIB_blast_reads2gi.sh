...
if [ $# -lt 4 ]
then
	echo "Usage:<candidate.fa> <blastdb.fa> <genus> <outputfile> <MIB_PATH>"
	exit
fi
...
$MIB_PATH/BLAST/makeblastdb -in $blastdb -input_type fasta -dbtype nucl -parse_seqids -out temp.$genus.blastdb
...
$MIB_PATH/BLAST/blastn -query $candidate -task blastn -reward 1 -penalty -1 -evalue 0.05 -db temp.$genus.blastdb -out $outputfile -outfmt 6 -max_target_seqs 1
