...
#upgrade note:
#
#	add the percentage of 16s-genus-reads to 16s-reads
#	solve the misclassfication of title of bacteria genus (E.X: ABCD is the genus_1. CD is the genus_2. MIB_0.1.0 will classfy ABCD into CD. A huge mistake)
#	solve the multi-hit problem
#
MIB_version="0.1.1"
...
BAC_16S_REF="$REF_PATH/DATABASE/SNAPDB/"
BAC_16S_FULL="$REF_PATH/DATABASE/BLASTDB/16s_full_name"

#SNAP parameters
SNAP_Edit_distance=12
...
declare -a software_list=("seqtk" "prinseq-lite.pl" "bowtie2" "MIB_distributionCalc.pl" "MIB_distributionPlot.py" "snap" "bowtie2" "vmtouch")
echo "#####################################################################################"
...
then
	...
	then
		...
		START_HUMAN=$(date +%s)
		bowtie2 -x "$host_DB" -r $inputfile.preprocessed -S $inputfile.preprocessed.human.sam -q -p $total_cores --mm
...
#max length supported by SNAP were 400
prinseq-lite.pl -trim_to_len 400 -fastq $inputfile.nohost.fastq -out_good $inputfile.nohost.mx400

######BLAST##############
if [ -f "$inputfile.nohost.fastq.sam" ]
	then	
		...
		echo -e "$(date)\t$0\t####Alignment to 16s Refseq/Gold RDP nucletide database####"
#		/home/t630/Desktop/VIP_ongoing_development/cluster_test/BLAST/blastn -query $inputfile.nohost.fasta -db $BAC_16S_REF -outfmt "6 qseqid sseqid qlen length pident qcovs qcovhsp mismatch gapopen qstart qend sstart send evalue bitscore" -out $inputfile.nohost.megablast -num_alignments 10 -perc_identity 95 -qcov_hsp_perc 97 -num_threads $total_cores
		###snap db cache check
		SNAP_db_cached=$(vmtouch -m500G -f "$BAC_16S_REF" | grep 'Resident Pages' | awk '{print $5}')
		...
		#snap single $snap_index $basef.fastq -o $basef.tmp.sam -t $cores -x -f -h 250 -d $SNAP_d_cutoff -n 25 > $basef.time.log
		snap single $BAC_16S_REF $inputfile.nohost.mx400.fastq -o $inputfile.nohost.fastq.sam -t $total_cores -x -f -h 250 -d $SNAP_Edit_distance -n 25 $SNAP_cache_option
	...
	Bac_16s_related_reads=`egrep -v "^@" $inputfile.nohost.fastq.sam | awk '{if($3!="*")print$1}' | uniq | wc -l | awk '{print$1}'`
	echo -e "16s_reads\t$Bac_16s_related_reads" >> $inputfile.reads_distribution
	egrep -v "^@" $inputfile.nohost.fastq.sam | awk '{if($3!="*")print}'> $inputfile.nohost.fastq.rmheader.sam
	rm -rf $inputfile.nohost.fastq.sam
	mv $inputfile.nohost.fastq.rmheader.sam $inputfile.nohost.fastq.sam
######add the original seqs##############

if [ -f "$inputfile.nohost.fastq.sam" ] 
then
	echo -e "$(date)\t$0\t####Restore the full length of reads being soft-cut during local alignment####"
	MIB_reStoreLength.sh $inputfile.nohost.fastq $inputfile.nohost.fastq.sam sam
else
	echo -e "$(date)\t$0\t####$inputfile.nohost.fastq.sam MISS"
	echo -e "$(date)\t$0\t####exit"	
	exit
fi
...
if [ -f "$inputfile.nohost.fastq.sam.addseq" ] 
then
	echo "$(date)\t$0\t####Taxnomoy Identification module for FAST mode result begins####"
	START_FAST_TAXI=$(date +%s)
	echo "$(date)\t$0\t####MIB_Tanc.sh $inputfile.nohost.fast_nucl.match.sam sam nucl $total_cores "$tax_DB"####"
	MIB_TanC.sh "$inputfile.nohost.fastq.sam.addseq" $total_cores "$tax_DB"
	...
	echo -e "$(date)\t$0\tFile missing: $inputfile.nohost.fastq.sam.addseq"
fi


######Report generation#################

if [ -f "$inputfile.nohost.fastq.sam.addseq.all.annotated" ] 
then
	echo "MIB_report.sh $inputfile.nohost.fastq.sam.addseq.all.annotated $inputfile $BAC_16S_FULL $MIB_DIR $Bac_16s_related_reads"
	MIB_report.sh $inputfile.nohost.fastq.sam.addseq.all.annotated $inputfile $BAC_16S_FULL $MIB_DIR $Bac_16s_related_reads
	...
	done < $inputfile.nohost.fastq.sam.addseq.all.annotated.genus.counttable.10
	...
	sort -t $'\t' -r -k6,6 -n ./$inputfile.report/temp.$inputfile.covreport | head -n 10 > ./$inputfile.report/temp.best20.$inputfile.temp
	echo -e "Species\tGenus\tGI\t%Coverage\tReads_hit\tReads_num\t%Reads_Ratio\tAverage depth of coverage" > ./$inputfile.report/temp.best20.$inputfile.title
	...
	echo -e "MIB_distributionPlot.py $inputfile.reads_distribution $inputfile reads"
	###reads_distribution.$inputfile.png
	cp reads_distribution.$inputfile.png $curdir/$inputfile.report/
	###familiy distribution top 5
	family_total=`wc -l $inputfile.nohost.fastq.sam.addseq | awk '{print$1}'`
	echo -e "Total\t$family_total" > $inputfile.family.top5
	sort -t $'\t' -r -n -k2,2 $inputfile.nohost.fastq.sam.addseq.all.annotated.family.counttable | head -n 5 >> $inputfile.family.top5
	...
	sort -t $'\t' -r -n -k2,2 $inputfile.nohost.fastq.sam.addseq.all.annotated.genus.counttable | head -n 10 > $inputfile.genus.top15
	awk '{print$1}' $inputfile.genus.top15 > $inputfile.genus.top15.list
	MIB_distributionPlot.py $inputfile.genus.top15 $inputfile genus
	cp genus_distribution.$inputfile.png $curdir/$inputfile.report/
	N=0
	mkdir $curdir/$inputfile.report/coverage_map_top10
	cd $curdir/$inputfile.report/
	while read genus
	do
		let "N = $N + 1"
		#echo -e "N is $N"
		mv -T covplot.$genus.$inputfile.png top$N.$inputfile.png
		mv top$N.$inputfile.png $curdir/$inputfile.report/coverage_map_top10/
		#cd $curdir
	done < $curdir/$inputfile.genus.top15.list
	cd $curdir/$inputfile.report/
