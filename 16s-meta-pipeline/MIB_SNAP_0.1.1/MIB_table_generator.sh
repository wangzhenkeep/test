...
MIB_create_tmp_table.pl -f $file_type $inputfile
...
MIB_taxonomy_classfication.pl $inputfile.tempsorted
########################################
...
sort -t $'\t'  -k 1,1 $inputfile.tempsorted.species.counttable > $inputfile.species.counttable
sort -t $'\t' -k 1,1 $inputfile.tempsorted.genus.counttable > $inputfile.genus.counttable
sort -t $'\t' -k 1,1 $inputfile.tempsorted.family.counttable > $inputfile.family.counttable
sort -t $'\t' -k 1,1 $inputfile.tempsorted.gi.counttable > $inputfile.gi.counttable
####################################3
