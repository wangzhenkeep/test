use warnings;
use strict;
use File::Basename;
use LWP::Simple;;


@ARGV > 0 or die "Usage : $0 command\n";
my $bin_path=dirname $0;
my $Pid=pop @ARGV;
my $command = "@ARGV";
my $jobID_file="/share/data/Pid2Jid/Pid2Jid.txt";
print "$command\n";
my @temp = `$command`;
my %task_id;
foreach (@temp){
    $task_id{$1}=1 if $_=~ /^your job (\d+)/i;
}
print "==>stat:\t",scalar localtime,"\n";
open OUT, ">>$jobID_file";
foreach(sort keys %task_id){
	print OUT "$Pid\t$_\n";
}
close OUT;
#sleep (1);
my $over=0;
my $rest=0;
until ($over++){
    @temp = `qstat`;
    foreach (@temp){
        if($_=~/^\s*(\d+)/ && exists $task_id{$1} ){
                        $rest++;
             $over=0;
        }
    }
        sleep (10);
        print "$rest work waiting...\n";
        $rest=0;

}
print "==>end :\t",scalar  localtime,"\n";
#$Pid=~s/ProjectID//;
#my $last_url="www.celloud.org/project!projectRunOver?projectId=".$Pid;
#my $user_agent = new LWP::UserAgent;
#my $request = new HTTP::Request('GET', $last_url);
#print "********$last_url\n";
#my $response = $user_agent->request($request);
$Pid=~s/ProjectID//;
#my $last_url="http://www.celloud.org/project!projectRunOver?projectId=".$Pid;
#http://www.celloud.cn/task!runOver?projectId=参数1&dataNames=参数2

my $last_url="http://www.celloud.cn/task!runOver?projectId=".$Pid."&dataNames=".$;


my $content = get($last_url);
