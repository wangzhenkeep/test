...
while (<IN>) {
	...
	print SH "perl $binpath/../High_PPI/Sureplex/Sureplex_pipeline_600k_Y_split_spark_luohui_port.pl $temp[0] $binpath/../DataBase/hg19.fasta $linshi_path/$temp $temp[2]\n";
	print SH "java -jar $binpath/batik-1.6/batik-rasterizer.jar $linshi_path/$temp/$temp\.*.svg\n";
	print SH "perl $binpath/log2xls.pl $linshi_path/$temp/$temp.log $linshi_path/$temp/$temp.Statistics $linshi_path/$temp/$temp.xls $file_name 0 \n";
