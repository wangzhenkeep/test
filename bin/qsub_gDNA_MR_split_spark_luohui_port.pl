...
while (<IN>) {
	...
	#print SH "echo \"start*****gDNA_pipeline_600k_qsub_spark_luohui_port.pl\"\ndate\n";
	print SH "perl $binpath/../High_PPI/gDNA/gDNA_pipeline_600k_qsub_spark_luohui_port.pl $temp[0] $binpath/../Database/hg19.fasta $linshi_path/$temp  $temp[2]\n";
	#print SH "echo \"start*****svg to png\"\ndate\n";
	#print SH "java -jar $binpath/batik-1.6/batik-rasterizer.jar $linshi_path/$temp/$temp\.*.svg\n";
	print SH "perl $binpath/svg2png.pl $linshi_path/$temp\n";
	#print SH "echo \"log2xls\"\ndate\n";
	print SH "perl $binpath/log2xls.pl $linshi_path/$temp/$temp.log $linshi_path/$temp/$temp.Statistics $linshi_path/$temp/$temp.xls $file_name 3.5\n";
	#print SH "echo \"start*****rm bam file\"\ndate\n";
	print SH "rm $linshi_path/$temp/*.bam\n";
	#print SH "echo \"start*****cp all file\"\ndate\n";
	print SH "mkdir $result_path/$temp\n";
	print SH "chmod -R 777 $result_path/$temp\n";
	print SH "cp -r $linshi_path/$temp $result_path/\n";
	print SH "cp -r $linshi_path/$sh* $result_path/\n";
	print SH "rm -rf $linshi_path/$sh*\n";
	...
	my $pid=`ps x | grep \"sh $linshi_path/$sh\" | awk \'{print \$1}\'`;
	print "$pid\n";
}
close IN;
